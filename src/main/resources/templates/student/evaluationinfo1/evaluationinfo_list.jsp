...
<div class="row" style="padding:15px; padding-top:0px; " align="right">
	<display:table class="table  table-condensed table-striped" name="list" id="cc"  pagesize="10" requestURI="evaluationxueqi/list.do">
		  ...
		  <c:if test="${cc.evaluationContent=='一般'}">
		   <%b++; %>
		</c:if>
		  <c:if test="${cc.evaluationContent=='不满意'}">
		   <%c++; %>
		</c:if>
		  <c:if test="${cc.evaluationContent=='特别不满意'}">
		   <%d++; %>
		</c:if>
...
%>
总记录数：<%=count %>&nbsp; &nbsp;其中： 
满意：<%=(int)(a*0.1/count*1000) %>% &nbsp; &nbsp;
一般：<%=(int)(b*0.1/count*1000) %>% &nbsp; &nbsp;
不满意：<%=(int)(c*0.1/count*1000) %>% &nbsp; &nbsp;
特别不满意：<%=(int)(d*0.1/count*1000) %>% &nbsp; &nbsp;
